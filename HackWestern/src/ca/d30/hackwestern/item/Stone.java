...
public class Stone extends Item {
	public Stone(int x, int y) {
		super("Stone", "Makes for a fine castle.", 0, item.getSprite(2, 2));
		this.x = x;
		this.y = y;
	}
	...
	public void tick() {
		...
		if(distanceToPlayer() < 8) {
			if(Main.instance.player.inv.addItem(this)) {
				Main.instance.player.numbers.add(new Num("+1 Stone", x, y, Main.instance.player, false));
				inInventory = true;
			}
		}
	}
	...
	public void use(Mob m) {
		...
		if(t.id == Tile.GRASS.id) {
			if(p.inv.removeItem(this)) w.setID(tX, tY, Tile.STONE.id);
		}
	}
}
